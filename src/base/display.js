...
export const displayTable = () => {
  ...
  for (
    ...
    $("<td></td>", {
      ...
      .text(entries[i].data("id"))
      .appendTo(row);
    $("<td></td>", {
      style: "color:gainsboro;text-decoration:underline",
      onclick: `click.room(${JSON.stringify(entries[i].data("id"))});`,
    })
      .text(entries[i].data("name"))
      .appendTo(row);
    $("<td></td>", {
      onclick: `click.room(${JSON.stringify(entries[i].data("id"))});`,
    })
      .text(entries[i].data("areaName"))
      .appendTo(row);
  }
  ...
  if (Math.ceil(displayEntries.length / pageBreak) > pageIndex + 1) {
    pagination = $("<span></span>", {
      ...
      `Displaying ${startIndex + pageBreak} of ${displayEntries.length}.`
    );
    pageIndex++;
    $("<span></span>", {
      style: "color:Goldenrod",
    ...
    pagination = $("<span></span>", {
      style: "color:Goldenrod",
    }).text(`Displaying ${displayEntries.length} of ${displayEntries.length}.`);
  }
